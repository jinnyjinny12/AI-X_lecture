...
#-----------------------------
window_width = 1920 #1280 #960 #1024
window_height= 1080 #720 #540 #768
#-------------------------------
...
print("Pose detection 시작")  # 디버깅 로그 추가
            ...
            if enable_segmentation:
                ...
                if not success:
                    ...
                    capBackground_yoga.set(1, 0)
                     ...
                     # Resize background frame to match the camera frame size (img)
                    img_back_resized = cv2.resize(img_back, (img.shape[1], img.shape[0]), interpolation=cv2.INTER_CUBIC)

                    # Create a resized bg_image using the resized img_back
                    bg_image = np.zeros(img.shape, dtype=np.uint8)
                    bg_image[:] = img_back_resized

                    # Merge the segmentation mask with the background
                    img = np.where(condition, img, bg_image)
                    
            else:
                print("Error: Segmentation mask is None. Skipping background overlay.")
            ...
            if wtimer_active:
                ...
                if (lhand_in_wtimer or rhand_in_wtimer) and not (lhand_in_ptimer or rhand_in_ptimer):
                    ...
                    if hand_in_wtimer_curr_time >= 1 and not wtimer_pressed:
                        ...
                        wtimer_tapped = True
                        print("이거 진행되는 거니?")
                        exit_butt_active = False #변경
                        cont_butt_active = False #변경
            ...
            if yoga_butt_active:
                ...
                if (lhand_in_yoga_butt or rhand_in_yoga_butt) and not (lhand_in_workout_butt or rhand_in_workout_butt):
                    ...
                    if hand_in_yoga_butt_curr_time >= 1 and not yoga_butt_pressed:
                        ...
                        yoga_butt_active = True ##변경
                        workout_butt_active = True ##변경
